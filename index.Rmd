...
library(scales)    # For proper number formatting
```
...
```{r}
# renaming data to avoid overwriting
df <- monthly_us_data

# Reshape data for Cases and Deaths into long format
df_long_counts <- df %>%
  pivot_longer(cols = c(cases, deaths),        # Columns to pivot into long format
               names_to = "Metric",            # New column for the metric names: 'cases' and 'deaths'
               values_to = "Count") %>%        # New column for the corresponding values
  select(-c(us_cases_rate, us_deaths_rate))    # Drop rate columns (not needed here)

# Reshape data for Case Rates and Death Rates into long format
df_long_rates <- df %>%
  pivot_longer(cols = c(us_cases_rate, us_deaths_rate),   # Columns to pivot into long format
               names_to = "rate_type",                    # New column for rate names: 'us_cases_rate' and 'us_deaths_rate'
               values_to = "rate") %>%                    # New column for the corresponding rate values
  select(-c(cases, deaths))                               # Drop cases and deaths columns (not needed here)

```


```{r, fig.width= 10, fig.height=8}
# Plot the Cases and Deaths with side-by-side bars (dodged) for each month and year
ggplot(df_long_counts, 
       aes(x = month, y = Count, fill = Metric)) +    # Set x-axis, y-axis, and fill color
  geom_bar(stat = "identity", position = "dodge") +   # Use dodged bars to place bars side by side
  facet_wrap(~ year, scales = "free") +               # Create separate plots (facets) for each year
  scale_y_continuous(labels = label_comma()) +        # Format y-axis values with commas
  labs(title = "National-Level Monthly COVID-19 Cases and Deaths by Year", 
...
```{r, fig.width= 10, fig.height=8}
# Plot the Monthly Rates of COVID-19 Cases and Deaths by Year
ggplot(df_long_rates, 
       aes(x = month, y = rate, fill = rate_type)) +  
  geom_bar(stat = "identity", position = "dodge") +  # Use side-by-side (dodged) bars for rate types
  facet_wrap(~ year, scales = "free") +              # Create separate panels (facets) for each year; scales adjust independently
  scale_y_continuous(labels = label_comma()) +       # Format y-axis values with commas for better readability
  labs(title = "National-Level Monthly Rates of COVID-19 Cases and Deaths by Year", 
       x = "Month",                                                  # Label for x-axis
       y = "Rate",                                                   # Label for y-axis
       fill = "Rate Type") +                                         # Legend title for 'rate_type'
...
fig1 <- plot_geo(subset1, locationmode = 'USA-states') %>%        # Create a US map with state boundaries
  ...
  layout(title = list(
      text = "U.S. State Map of COVID-19 Cases (March 23, 2023)<br><sup>Hover over the map to see more details</sup>",
...
plot1 <- ggplot(data = subset1) +
  ...
  labs(x = "State", y = "Covid Cases", title = "Covid Cases per State") +
  coord_flip() +
  theme_classic() +
...
plot2 <- ggplot(data = subset1) +
  geom_bar(aes(x = reorder(state, deaths),
               y = deaths, fill = state),
           stat = "identity", show.legend = FALSE) +
  labs(x = "State", y = "Covid Deaths", title = "Covid Deaths per State") +
  coord_flip() +
  theme_classic() +
  theme(axis.text.y = element_text(size = 12, face = "bold")) # Increase font size and bold state names
