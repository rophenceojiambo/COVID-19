...
monthly_racial_aggregate <- combined_monthly_data %>%
  group_by(date) %>%
...
monthly_racial_aggregate %>%
  ...
  row_spec(0, bold = TRUE, background = "#f5f5f5")  # Highlight the header row

```

```{r}
# Reshape the data to long format for easy plotting
monthly_racial_aggregate_long <- monthly_racial_aggregate %>%
  pivot_longer(cols = c(Cases_White, Cases_Black, Cases_Asian, Cases_AIAN,
                        Cases_NHPI, Cases_Multiracial),
               names_to = "racial_group",
               values_to = "cases") %>%
  # Remove "Cases_" prefix
  mutate(racial_group = str_remove(racial_group, "Cases_")) %>%
  arrange(date, cases)
...
date_breaks <- seq(min(monthly_racial_aggregate$date), 
                   max(monthly_racial_aggregate$date), by = "1 month")
...
# Plot the data
ggplot(monthly_racial_aggregate_long, aes(x = date, y = cases, group = racial_group, color = racial_group)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_color_manual(values = custom_colors) +
  scale_x_date(date_labels = "%m/%Y", breaks = date_breaks, expand = c(0.01, 0.01)) +
  scale_y_continuous(labels = scales::comma) +
  labs(title = "Cumulative Monthly COVID-19 Cases by Racial Group",
       x = "Month/Year",
       y = "Number of Cases",
       color = "Racial Group") +
  theme_minimal() +
